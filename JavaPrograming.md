...
- 상속의 관계 : IS - A 관계 / 일반적인 개념과 구체적인 개념과의 관계이다
  상위 클래스 : 일반적인 개념 클래스 ex) 포유류
  하위 클래스 : 구체적인 개념 클래스 ex) 사람, 원숭이 ..
  단순히 코드를 재사용 하는 목적으로 사용하지 않는다.

- HAS - A 관계 / 한 클래스가 다른 클래스를 소유한 관계
  코드 재사용의 한 가지 방법으로 Student가 Subject를 포함한 관계

- 다운 캐스팅 : instanceof 
  하위 클래스가 상위 클래스로 형 변환 되는것은 묵시적으로 이루어지지만
  다시 원래 자료형인 하위 클래스로 형 변환을 하려면 명시적으로 다운 캐스팅을
  해주어야 한다. 예약어는 instacneof

## 추상 클래스와 템플릿 메소드

- 추상 클래스란 추상 메소드를 포함한 클래스, 추상 메소드는 구현코드 없이
  메소드의 선언만 있음. 
  ex) abstract int add(int x, int y); 선언만 있는 추상 메소드
    int add(int x, int y){   } {}부분이 구현 내용이므로 추상이 아님

- 추상 클래스는 new로 인스턴스화 할 수 없다.

- 템플릿 메소드 : 추상 메소드나 구현된 메소드를 활용하여 전체 기능의 흐름을
  정의하는 메소드이다. final로 선언하면 하위 클래스에서 재정의 하지 못한다.

- 프레임 워크에서 많이 사용되는 설계 패턴이다

- 추상 클래스로 선언된 상위 클래스에 템플릿 메소드를 활용하여 전체적인
  흐름을 정의 하고 하위 클래스에서 다르게 구현되어야 하는 부분은 추상
  메소드로 선언하여 하위 클래스가 구현하도록 한다.

- final 예약어 : final 변수는 값이 변경될 수 없는 상수이다.
  오직 한 번만 값을 할당할 수 있으며 하위 클래스에서 재정의 할 수 없다.
  또 final 클래스는 더 이상 상속되지 않는다.
